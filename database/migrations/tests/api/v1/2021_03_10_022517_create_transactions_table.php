...
class CreateTransactionsTable extends Migration
{
    ...
    public function up()
    {
        Schema::connection('unit_test_payment_pgsql')->create('transactions', function (Blueprint $table) {
            $table->bigIncrements('t_id');
            $table->bigInteger('t_xref_fg_id')->index()->comment('tlsconnect fg_id');
            $table->string('t_transaction_id')->unique()->comment('transaction id generate by payment system');
